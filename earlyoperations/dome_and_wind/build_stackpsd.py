...
CHANNELS = {"lsst.sal.MTM1M3.imsData": "zPosition",
...
async def main():
    ...
    for k, ng in enumerate(nights, 1):
        ...
        t1 = Time((sub.index.max() + pd.to_timedelta(sub["exp_time"].max(), unit="s")).to_pydatetime()) + PAD
        for topic, ch in CHANNELS.items():
            try:
                raw = await client.select_time_series(topic, fields=[ch], start=t0, end=t1)
            except Exception as e:
                print(f"   {ng} {ch} err {str(e)[:50]}"); continue
            if raw.empty:
                continue
            if raw.index.tzinfo is None:
                raw.index = raw.index.tz_localize("UTC")
            rate = 1.0/np.median(np.diff(raw.index.astype(np.int64)/1e9)) if len(raw) > 3 else 50.0
            for t_exp, row in sub.iterrows():
                seg = raw.loc[t_exp:t_exp + pd.to_timedelta(row["exp_time"], unit="s")]
                ...
                for lo, hi in STACK_WIND_BINS:
                    if lo <= w < hi:
                        accum.setdefault((ch, lo, hi), []).append((f, pxx)); break
    ...
    for (ch, lo, hi), recs in accum.items():
        ...
        for fi, pi in zip(fgrid, med):
            rows.append({"channel": ch, "wind_lo": lo, "wind_hi": hi,
