...
def stratified_sample(night_df):
    ...
    nd["_score"] = (nd["efd_wind_speed"].rank(pct=True)
                    + 0.5 * nd.get("into_wind", False).astype(float))
    # spread across wind bins: split into PER_NIGHT quantile slots, take top-score each
    nd["_slot"] = pd.qcut(nd["efd_wind_speed"].rank(method="first"),
                          q=min(PER_NIGHT, len(nd)), labels=False, duplicates="drop")
    return nd.sort_values("_score", ascending=False).groupby("_slot", observed=True).head(1)

df = df.groupby("night", group_keys=False).apply(stratified_sample)
print(f"[vms] wind-stratified sample: {len(df)} exposures across {df['night'].nunique()} nights "
      f"(~{PER_NIGHT}/night)")
...
def unpack_axis(seg, ax):
    ...
    cols = [f"acceleration{ax}{i}" for i in range(VMS_PACK) if f"acceleration{ax}{i}" in seg]
    if not cols:
        return np.array([])
...
async def main():
    ...
    accum = {}   # (band-key placeholder) → list of (f,pxx) for stacked PSD (per wind bin)
    nights = sorted(df["night"].unique())
    for k, ng in enumerate(nights, 1):
        ck = NIGHT_DIR / f"vms_{ng}.parquet"
        if ck.exists():
            print(f"[{k:3d}/{len(nights)}] {ng}: cached"); continue
        sub = df[df["night"] == ng]
        rows = []
        ...
        async def fetch_exp(ts, te):
            for attempt in range(3):
                try:
                    return await client.select_time_series(VMS_TOPIC, fields=XCOLS + ["sensor"],
                        start=Time(ts.to_pydatetime()), end=Time(te.to_pydatetime()), index=VMS_INDEX)
                except Exception:
                    if attempt == 2:
                        return pd.DataFrame()
        ...
        if True:
            for t_exp, r in sub.iterrows():
                ...
                for ax in AXES:
                    ...
                    psd_acc = pxx if psd_acc is None else (psd_acc[:len(pxx)] + pxx[:len(psd_acc)])
                if not all_samps or psd_acc is None:
                    continue
                row["vms_accel_rms"] = float(np.std(np.concatenate(all_samps)))
                for lo, hi in FREQ_BANDS:
                    row[f"vms_accel_rms_{lo:g}_{hi:g}Hz"] = band_rms(fgrid, psd_acc, lo, hi)
                if len(fgrid) > 2:
                ...
                for lo_w, hi_w in STACK_WIND_BINS:
                    if lo_w <= w < hi_w:
                        accum.setdefault((lo_w, hi_w), []).append((fgrid, psd_acc)); break
        pd.DataFrame(rows).to_parquet(ck)
        print(f"[{k:3d}/{len(nights)}] {ng}: {len(rows)} exp", flush=True)

    # assemble per-exposure VMS metrics
    frames = [pd.read_parquet(p) for p in sorted(NIGHT_DIR.glob("vms_2*.parquet")) if p.stat().st_size > 0]
    ...
    for (lo_w, hi_w), recs in accum.items():
        ...
        for fi, pi in zip(fg, med):
            srows.append({"wind_lo": lo_w, "wind_hi": hi_w, "freq_hz": fi,
                          "psd_median": pi, "n_exp": len(recs)})
    if srows:
