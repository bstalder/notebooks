...
NUMERIC_COLS = [
    ...
    "mount_motion_image_degradation","mount_motion_image_degradation_az",
    "mount_motion_image_degradation_el","mount_motion_image_degradation_rot",
...
def coerce(df):
    ...
    if "can_see_sky" in df:  # bool/object mix → normalize to nullable boolean-ish string kept as str
        df["can_see_sky"] = df["can_see_sky"].map(
            lambda v: True if str(v).lower() in ("true","1","t") else
                      (False if str(v).lower() in ("false","0","f") else np.nan))
    return df
