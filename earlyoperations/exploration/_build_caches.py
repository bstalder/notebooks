...
def build_dia_cache():
    ...
    for shard_id in range(N_SHARDS):
        ...
        print(f"  shard {shard_id}: [{start}:{end}] ({len(shard_refs)} files)...", flush=True)
        results = []
    ...
    frames = [pd.read_parquet(CACHE_DIR / f"shard_{s:02d}.parquet") for s in range(N_SHARDS)]
...
def build_ss_cache():
    ...
    for shard_id in range(SS_N_SHARDS):
        ...
        if shard_file.exists():
            print(f"  shard {shard_id}: already done")
            continue
        start, end = shard_id * shard_size, min((shard_id + 1) * shard_size, len(ss_refs))
        shard_refs_slice = ss_refs[start:end]
        print(f"  shard {shard_id}: [{start}:{end}] ({len(shard_refs_slice)} files)...", flush=True)
        results = []
    ...
    frames = [pd.read_parquet(SS_CACHE_DIR / f"shard_{s:02d}.parquet") for s in range(SS_N_SHARDS)]
    df = pd.concat(frames, ignore_index=True)
