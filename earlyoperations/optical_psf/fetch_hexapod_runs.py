...
FIELDS = ["salIndex", "position2", "position3", "position4",
          "demand2", "demand3", "demand4"]
...
async def main():
    ...
    for rid, rr in sel.iterrows():
        t0 = Time(pd.Timestamp(rr["t0"]).tz_convert("UTC").tz_localize(None) - pd.Timedelta(seconds=PAD_S))
        t1 = Time(pd.Timestamp(rr["t1"]).tz_convert("UTC").tz_localize(None) + pd.Timedelta(seconds=PAD_S))
        for idx in (1, 2):
            try:
                df = await c.select_time_series(TOPIC, fields=FIELDS, start=t0, end=t1, index=idx)
            ...
            r = df[["position2", "position3", "position4", "demand2", "demand3", "demand4"]].resample("2s").median().dropna(how="all")
            r["run"] = rid
            r["salIndex"] = idx
