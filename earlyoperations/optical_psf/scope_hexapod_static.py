...
async def main():
    ...
    cmd = await c.select_time_series(
        ...
        fields=["z", "salIndex"], start=t0, end=t1, index=1)
    ...
    cm = await c.select_time_series(
        ...
        fields=["enabled", "salIndex"], start=t0, end=t1, index=1)
    ...
    df[["visitId", "day_obs", "science_program", "obs_start",
        "quiet_window_s", "comp_off"]].to_parquet(OUT)
